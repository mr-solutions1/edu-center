...
const GroupsPage = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data) => editingGroup ? groupApi.update(editingGroup._id, data) : groupApi.create(data),
    onSuccess: () => {
    ...
    { header: 'عدد الطلاب', cell: (row) => row.students?.length || 0 },
    {
      ...
      cell: (row) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={() => { setEditingGroup(row); reset(row); setOpen(true); }}>
  ...
  return (
    <div className="space-y-6 text-right" dir="rtl">
      <PageHeader title="المجموعات الدراسية" description="إدارة المجموعات والطلاب">
        <Button onClick={() => { setEditingGroup(null); reset({}); setOpen(true); }} className="gap-2">
      ...
      >
        <form id="group-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2 text-right">
            ...
            <Input {...register('name')} placeholder="مثال: مجموعة التقوية - رياضيات" />
          </div>
          <div className="space-y-2 text-right">
            <Label>الدورة</Label>
          ...
          <div className="space-y-2 text-right">
            ...
            <select {...register('teacherId')} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
              ...
              {teachers?.data?.map((t) => <option key={t._id} value={t._id}>{t.employeeCode} - {t.userId?.firstName}</option>)}
            </select>
          </div>
        </form>
      </FormDialog>
