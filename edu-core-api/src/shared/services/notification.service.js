...
class NotificationService {
  ...
  async notify(payload) {
    ...
    logger.info(
      `Notification triggered for user ${userId || 'unknown'}: "${title}"`
    );
    ...
    if (
      userId &&
      (!contactInfo.email || !contactInfo.phone || !activeTenantId)
    ) {
      try {
        ...
        logger.error(
          `[NotificationService] Failed to retrieve user details: ${err.message}`
        );
      }
    }
    ...
    try {
      ...
      logger.error(
        `[NotificationService] Failed to load provider factory for tenant ${activeTenantId}: ${err.message}`
      );
    ...
    results.forEach((res) => {
      if (res.status === 'rejected') {
        logger.error(
          `[NotificationService] Delivery dispatch failed: ${res.reason}`
        );
      } else {
        const { name, type: mode, result } = res.value;
        if (result && result.success === false) {
          logger.warn(
            `[NotificationService] Provider ${name} (${mode}) failed: ${result.error}`
          );
        } else {
          logger.info(
            `[NotificationService] Provider ${name} (${mode}) dispatched successfully.`
          );
        }
      }
    });
